resource "aws_lambda_function" "kinesis_lambda" {
  ...
  # Note: There is no need to output the iam_role arn from the iam.tf file, because iam.tf and main.tf are in the same module. Just use it.
  ...
  // This step is optional (environment), since it could be managed with CI/CD
  environment {
    variables = {
